...
# === 1. Formüllü versiyon → Aynı isimle kopyala
formullu_target_path = os.path.join(archive_folder, "coin_data_180days_top100.xlsx")
shutil.copy2(source_file, formullu_target_path)
print(f"📄 Formüllü dosya kopyalandı → {formullu_target_path}")

# === 2. Formülsüz versiyon → Market_Values_[tarih].xlsx adıyla oluştur
wb = load_workbook(source_file, data_only=True)
formulsuz_target_path = os.path.join(archive_folder, f"Market_Values_{folder_name}.xlsx")
wb.save(formulsuz_target_path)
wb.close()
print(f"📄 Formülsüz dosya oluşturuldu → {formulsuz_target_path}")
